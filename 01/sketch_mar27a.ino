/*
 * 본예제는 LED 포트번호가 순차정렬이 되어있다는 가정하에 작성되었습니다.
 */
...
void step05(int pin){
  ...
  if(pin%2==1){ //2의 배수가 아니면
    for(int k_in=(half+1); k_in<=forLast; k_in++) {
      digitalWrite(k_in,LOW); //3,4 꺼짐
    }
    for(int k_in=0; k_in<=half; k_in++) {
      digitalWrite(k_in,HIGH); //1,2 꺼짐
    }
  }else{ //2의 배수가  맞으면
    for(int k_in=0; k_in<=half; k_in++) {
      digitalWrite(k_in,LOW); //1,2 꺼짐
    }
    for(int k_in=(half+1); k_in<=forLast; k_in++) {
      digitalWrite(k_in,HIGH); //3,4 켜짐
    }
  } 
  delay(dTime);
}
...
void step06(int pin){
  ...
  if(forLast != pin){ //마지막이 아니면
    digitalWrite(pin_pre,LOW); //이전 LED를 꺼준다
    digitalWrite(pin,HIGH); //현재의 LED를 켜준다.
    digitalWrite(pin_next,HIGH); //다음의 LED를 켜준다.
  }else{ //마지막이면
    digitalWrite(pin,LOW); //현재의 LED를 꺼준다
    digitalWrite(pin_pre,LOW); //이전의 LED를 꺼준다
  }
  delay(dTime);
}
